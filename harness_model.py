...
class NodeType(str, Enum):
    ...
    BRANCH_POINT = "branch_point"    # interrupts an edge; a future anchor for connections/fasteners
    LAYOUT_POINT = "layout_point"    # routing/visual only — e.g. to carve out space on a 1:1 forming board


# How close two BRANCH_POINT nodes must be (in scene units, i.e. mm) for
# dragging one near the other to merge them. A single shared constant so
# the live drag-time preview (harness_view.py) and the actual merge
# trigger (harness_controller.py) always agree on the same threshold.
# Intentionally adjustable later — e.g. exposed as a setting.
BRANCH_MERGE_DISTANCE_MM = 20.0
